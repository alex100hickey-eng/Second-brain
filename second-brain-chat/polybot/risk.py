...
class RiskManager:
    ...
    def allow(self, sig, bankroll_usd: float | None = None):
        ...
        if existing + sig.size_usd > caps.max_per_market_usd + 1e-9:
            portfolio_refusal = f"market exposure ${existing:.0f}+${sig.size_usd:.0f} > cap"
        total = self.ledger.exposure_usd(sig.venue, None, live_only=live_like)
        if portfolio_refusal is None and total + sig.size_usd > caps.max_exposure_usd + 1e-9:
