...
def build_cmd(src: str, dst: str, text_png: str | None = None, text_pos: str = "top", trim_start: float = 0.0,
              ...
              hook_audio: str | None = None, hook_len: float = 0.0, text_seconds: float = 2.8) -> list:
    ...
    if png_idx is not None:
        ...
        chain += f";[v0][{png_idx}:v]overlay=(W-w)/2:{y}:enable='between(t,0,{text_seconds:.2f})',format=yuv420p[v]"
    else:
        chain += ";[v0]format=yuv420p[v]"
...
def make_variant(src: str, dst: str, text_hook: str, recipe: dict, hook_audio: str | None = None,
    ...
    try:
        cmd = build_cmd(src, dst, png, recipe.get("text_pos", "top"), recipe.get("trim_start", 0.0),
                        recipe.get("trim_end", 0.0), recipe.get("zoom", 1.0), dur, hook_audio, hook_len, text_seconds)
