...
(function (global) {
  ...
  function _mobileShell(deck, opts) {
    ...
    // Header: the reactor, big. On subpages it is the way back to the deck
    // (subpage.html passes reactorHref '/'), so it has to be an easy thumb
    // target, not a glyph — 112px is comfortably past the ~44px tap minimum
    // and reads as the anchor of the page rather than decoration.
    const head = document.createElement('div');
    head.className = 'hud-m-head';
    const core = HUD.coreReactor({ size: 112, href: opts.reactorHref || '/chat-classic',
    ...
    // Decorative pieces are dropped here. They were briefly banded between the
    // widgets (2026-07-31) to carry HUD_STYLE's density rule onto the phone, but
    // in the hand they read as clutter between the things Alex actually taps —
    // the density rule earns its keep on the desktop scene, not in a scroll column.
