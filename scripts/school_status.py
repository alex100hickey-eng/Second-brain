...
def ungraded(row) -> bool:
    ...
    assignments API. Such a row is prep, never a deadline: never OVERDUE,
    never DO NEXT, never in the due list — at most a one-line hint for the
    next class. A blank weight is unknown, not 0, so ECON103's graded "Reading
    N" rows (5 pts, late = zero) stay hard. Mirrored verbatim in
    second-brain-chat/school_data.py."""
    try:
...
def main():
    ...
    open_work = [a for a in assignments
                 if (a.get("status", "") or "").strip().lower() not in DONE]
    # Not-graded prep (weight_pct 0) is never late: the class it served has
    # happened, attendance is assumed, and there was nothing to hand in.
    prep = [a for a in open_work if ungraded(a)]
    open_work = [a for a in open_work if not ungraded(a)]
    overdue = sorted(
    ...
    # ---------- prep for the next class (ungraded) — one line per course, at most ----------
    # Canvas not_graded items (ACCT "Day N Reading" = WileyPlus section links):
    # nothing to submit, no points, so no OVERDUE and no DO NEXT. Still worth one
    # dim line — it is what the next class and its graded APQ draw on. Only the
    # nearest date shows; anything already past dropped silently with its class.
    by_course = {}
    for a in prep:
        d = date(a.get("due_date"))
        if d and today <= d <= horizon:
            by_course.setdefault((a.get("course") or "?").strip(), []).append((d, a))
    for code in sorted(by_course):
        items = sorted(by_course[code], key=lambda t: t[0])
        first = items[0][0]
        titles = "; ".join((a.get("title") or "")[:44] for d, a in items if d == first)
        print(f"  {color('prep', 'dim'):<18} {code:<10} {titles[:44]:<44} "
              f"{color(f'for {first:%b %d} · ungraded', 'dim')}")
