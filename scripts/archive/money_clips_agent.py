# ARCHIVED 2026-08-14 — retired, not deleted.
#
# This was "Money/Business Agent #1", from the shorts-channel idea that predates the
# August ad-creative plan. It ran as a Coolify scheduled task on the server, daily at
# 13:00 UTC, and worked fine: one Sonnet call, one Shorts concept, one Supabase row.
#
# It was retired because nothing ever consumed it. Its own docstring says the output is
# "for review before you feed it into Viewmax" — that feeding never happened, so it
# quietly accumulated weeks of unread concepts. Alex, 2026-08-14: "I've never really
# needed it, it was a trial thing at first."
#
# It also lived ONLY on the server and was never committed, so it couldn't be read or
# reviewed from the repo. A 2026-08-01 check concluded the task "does not exist" and
# told the next session not to look for it — it did exist, and kept running for another
# two weeks. That is the real reason this file is here: server-only code is invisible
# code. Archive it in the repo even when retiring it.
#
# The generated concepts are still in Supabase (agent_name = "money_clips_agent").
# To restore: re-add a Coolify scheduled task on the app resource, frequency 0 13 * * *,
# command: /opt/venv/bin/python3 money_clips_agent.py
